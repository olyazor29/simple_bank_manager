...
class ViewBalanceFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val callback = requireActivity().onBackPressedDispatcher.addCallback {
            findNavController().navigate(R.id.action_viewBalanceFragment_to_userMenuFragment)
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (userViewModel.balance.value == null) {
            val intent = (view.context as MainActivity).intent
            val balanceFromIntent = intent.extras?.getDouble("balance") ?: 100.0
            userViewModel.setBalance(balanceFromIntent)
        }
